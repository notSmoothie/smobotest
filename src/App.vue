        ...
        <v-main>
            <div class="grid-wrapper">
                <CharacterView
                    ...
                    :character="character"
                    :characterIds="
                        charactersIds[`${character.name.toLowerCase()}`]
                    "
                    :lastWed="lastWed"
                ></CharacterView>
                <CharacterView
                    ...
                    :rotated="
                        rotatedName == 'SB-addChar' ||
                        (lastFill && rotatedName == lastChar?.name)
                    "
                    :lastWed="lastWed"
            ...
            <v-card>
                ...
                <v-card-text>
                    ...
                    <div class="text-overline">
                        ...
                        <span class="text-primary font-weight-regular">{{
                            new Date(
                                `${updateInfo.manifest.date.split(" +")[0]}Z`
                            ).toLocaleDateString()
                        }}</span>
                    </div>
                    <br />
                ...
                <v-card-actions>
                    <v-btn variant="elevated" @click="updateDialog = false"
                        >Cancel</v-btn
                    >
                    <v-spacer></v-spacer>
                    <v-btn variant="elevated" color="primary" @click="update"
                        >Install</v-btn
                    >
                </v-card-actions>
            </v-card>
...
import { appDataDir, BaseDirectory as pathBase } from "@tauri-apps/api/path";
...
export default {
    ...
    async created() {
        ...
        try {
            console.log("pb", pathBase.App);
        } catch (e) {
            console.error("error", e);
        }
        if (
            !(await exists("chars.conf", { dir: BaseDirectory.AppLocalData }))
        ) {
            await writeFile("chars.conf", "", {
                dir: BaseDirectory.AppLocalData,
            });
        }
        this.characters = JSON.parse(
            await readTextFile("chars.conf", {
                dir: BaseDirectory.AppLocalData,
            })
        );

        if (!(await exists("id.conf", { dir: BaseDirectory.AppLocalData }))) {
            await writeFile("id.conf", "", { dir: BaseDirectory.AppLocalData });
        }
        this.charactersIds = JSON.parse(
            await readTextFile("id.conf", { dir: BaseDirectory.AppLocalData })
        );
        // await exists("id.conf", { dir: BaseDirectory.AppLocalData });
        // const ids = await readTextFile("id.conf", { dir: BaseDirectory.AppLocalData });
        // console.log(chars);
        // console.log(ids);
        ...
        const unlistenTwo = await appWindow.onCloseRequested(async (event) => {
            event.preventDefault();
            console.log(this.characters);
            await writeTextFile(
                "chars.conf",
                this.characters.reduce((acc, el) => {
                    acc = acc += `\n${el.name}`;
                    return acc;
                }, []),
                {
            ...
            await writeTextFile("id.conf", JSON.stringify(this.charactersIds), {
                dir: BaseDirectory.AppLocalData,
            });
        });
    ...
    methods: {
        ...
        async draggDel() {
            delete this.charactersIds[
                `${this.characters[this.draggIdx].name.toLowerCase()}`
            ];
